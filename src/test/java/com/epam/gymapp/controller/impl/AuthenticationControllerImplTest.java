...
public class AuthenticationControllerImplTest {
  ...
  void authenticate_Success() throws Exception {
    ...
    String token = JwtTokenTestUtil.generateToken(new HashMap<>(), UserTestUtil.getTraineeUser1());
  ...
  void logout_Success() throws Exception {
    // Given
    JwtToken jwtToken = JwtTokenTestUtil.getNewJwtTokenOfTrainee1();

    // When
    when(jwtTokenRepository.findByToken(any())).thenReturn(Optional.of(jwtToken));

    ResultActions result = mockMvc.perform(put("/api/auth/logout")
        .header(HttpHeaders.AUTHORIZATION, "Bearer " + jwtToken.getToken()));
